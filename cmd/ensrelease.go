...
var ensReleaseCmd = &cobra.Command{
    ...
    gramreal ens release --domain=enstest.eth --passphrase="my secret passphrase"

Multiple domains can be released with a single command (note this still creates one transaction for each name).  For example:

    gramreal ens release --domains=mydomain1.eth&&mydomain2.eth --passphrase="my secret passphrase"
