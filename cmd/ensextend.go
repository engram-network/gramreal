...
var ensExtendCmd = &cobra.Command{
    ...
    gramreal ens extend --domain=enstest.eth --value="0.1 Ether" --passphrase="my secret passphrase"

Multiple domains can be extended with a single command (note this still creates one transaction for each name).  For example:

    gramreal ens extend --domains="mydomain1.eth&&mydomain2.eth" --value="0.1 Ether" --passphrase="my secret passphrase"
