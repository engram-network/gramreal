...
var ensRegisterCmd = &cobra.Command{
    ...
    gramreal ens register --domain=enstest.eth --passphrase="my secret passphrase"
...
Multiple domains can be registered with a single command (note this still creates one transaction for each name).  For example:
    ...
    gramreal ens register --domains=mydomain1.eth&&mydomain2.eth --passphrase="my secret passphrase"
